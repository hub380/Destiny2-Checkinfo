...
function craftingSourceKind(source: string) {
  if (source.startsWith('突袭') || source.includes('突袭')) return 'raid';
  if (source.startsWith('地牢') || source.includes('地牢')) return 'dungeon';
  if (source.startsWith('篇章活动') || source.includes('篇章')) return 'episode';
  if (source.startsWith('赛季活动') || source.includes('赛季')) return 'season';
  return 'other';
}

function countCraftingItems(groups: CraftingSourceGroupDto[]) {
    ...
    return groups
      .map((group) => ({
        ...
        items: group.items.filter((item) => {
          ...
          return name.includes(needle) || type.includes(needle) || group.source.toLowerCase().includes(needle);
        })
      }))
          ...
          <div className={cn('crafting-modal-controls')}>
            ...
            <p className={cn('crafting-modal-meta')}>
              {filterActive
                ...
                : `${formatNumber(groups.length)} 个来源 · ${formatNumber(totalCount)} 件装备`}
            </p>
          </div>
        </div>
        <div className={cn('career-modal-scroll crafting-modal-scroll')}>
          <div className={cn('crafting-collection-board')}>
            {visibleGroups.length ? visibleGroups.map((group) => <CraftingSourceGroup group={group} key={group.key} />) : <div className={cn('detail-loading')}>没有匹配的锻造装备</div>}
          </div>
        </div>
      </div>
    </div>
