  ...
  async function runQuery(rawName: string) {
    ...
    try {
      ...
      await Promise.allSettled([
        getCareerDetails(baseRequest).then((details) => {
          setCareer((current) => current ? {
            ...current,
            detailLoading: false,
            details: details.details,
            cache: { ...(current.cache || {}), ...(details.cache || {}) }
          } : current);
        }).catch((err) => setCareer((current) => current ? { ...current, detailLoading: false, detailError: err.message } : current)),
        loadEndgameMode(baseRequest, 'raid'),
        loadEndgameMode(baseRequest, 'dungeon'),
        loadEndgameMode(baseRequest, 'pvp')
      ]);
      setNotice('');
    } catch (err: any) {
  ...
  async function loadEndgameMode(baseRequest: any, mode: 'raid' | 'dungeon' | 'pvp') {
    try {
      ...
      setCareer((current) => current ? {
        ...
        endgameErrors: { ...(current.endgameErrors || {}), [mode]: err.message }
      } : current);
    }
  }
...
function mergeEndgameCareer(career: CareerSummaryDto, payload: any, mode: string): CareerSummaryDto {
  const endgame = payload.endgame || payload;
  const loading: Record<string, boolean> = career.endgameLoading && typeof career.endgameLoading === 'object'
...
createRoot(document.getElementById('root')!).render(<CareerPage />);
