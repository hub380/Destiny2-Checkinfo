...
import { getGuide, getGuides } from '../api';
import type { GuideDetailDto, GuideIndexDto, GuideSummaryDto, JsonRecord } from '../types';
import { Header, Notice, SearchIcon, css, dateOnly, uiClasses } from '../ui';
import '../global.css';
...
function GuidesPage() {
  ...
  async function openGuide(item: GuideSummaryDto, silent = false) {
  ...
  return (
    <div className={cn('app-shell')}>
      <Header title="Destiny 2 攻略/资讯" subtitle="Raid、地牢、地图与机制资料库" current="guides" />
      <main className={cn('guides-layout')}>
        <section className={cn('panel guides-list-panel')}>
          <div className={cn('panel-header')}>
            <div>
              <h2>攻略库</h2>
              <p>{loading ? '加载中' : `${visibleItems.length} / ${items.length} 篇`}</p>
            </div>
        ...
        <section className={cn('panel guide-detail-panel')}>
          {detailLoading ? (
            <div className={cn('empty-state')}>
              <b>正在加载攻略</b>
...
function GuideCard({ item, active, onOpen }: { item: GuideSummaryDto; active: boolean; onOpen: () => void }) {
  return (
    <button className={cn(`guide-card ${active ? 'active' : ''}`)} type="button" onClick={onOpen}>
...
createRoot(document.getElementById('root')!).render(<GuidesPage />);
